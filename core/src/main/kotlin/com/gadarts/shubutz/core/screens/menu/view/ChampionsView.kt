...
class ChampionsView(
    ...
    private fun addArrowButton(
        ...
        arrowButton.addListener(object : ClickListener() {
            override fun clicked(event: InputEvent?, x: Float, y: Float) {
                ...
                val difficulties = BombGameModes.values()
                val indexOf = difficulties.indexOf(BombGameModes.valueOf(currentDisplayed.name))
    ...
    private fun shouldUpdateCache() =
        champions.size < BombGameModes.values().size || TimeUtils.timeSinceMillis(lastChampionsFetch) >= CACHE_TTL
    ...
    private fun viewReady(
        ...
        if (champions.size == BombGameModes.values().filter { it.leaderboardsId != null }.size) {
    ...
    private fun randomDifficulty(): BombGameModes {
        val difficulties = BombGameModes.values()
    ...
    companion object {
        ...
        private val champions = HashMap<BombGameModes, Champion>()
    }
}
